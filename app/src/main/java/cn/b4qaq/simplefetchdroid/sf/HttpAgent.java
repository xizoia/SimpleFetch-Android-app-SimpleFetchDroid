...
public final class HttpAgent {
    ...
    public static Response execute(String method, String url, Map<String, String> headers,
        ...
        HttpURLConnection conn = open(method, url, headers, timeoutMs, body);
        try {
    ...
    private static HttpURLConnection open(String method, String url,
        ...
        // 与 SimpleFetch 插件行为对齐（v1.1.5）：带 body 且未声明 Content-Type 时，
        // 按 body 首字符判别：{ 或 [ 视为 JSON，否则视为 application/x-www-form-urlencoded；
        // 否则服务器无法解析 body（如 api.b4qaq.cn 返回 500）。
        if (!hasContentType && body != null && body.length > 0
                && !"GET".equalsIgnoreCase(method) && !"HEAD".equalsIgnoreCase(method)) {
            String ct = isJsonBody(body) ? "application/json;charset=UTF-8"
    ...
    /** body 首个非空白字节为 { 或 [ 时按 JSON 处理（与 SimpleFetch 插件一致）。 */
    private static boolean isJsonBody(byte[] body) {
        for (byte b : body) {
            if (b == ' ' || b == '\t' || b == '\n' || b == '\r') continue;
            return b == '{' || b == '[';
        }
