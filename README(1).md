...
- **v1.5.10**：已桥接快应用再次上线即「断开 → 等 3 秒 → 连接」；8 秒冷却窗口防自触发死循环。
- **v1.5.9**：修复再次上线的反复断开重连循环（仅离线后重连）。
- **v1.5.8**：修复「最近应用」点击无反应；再次上线改「先断开再连接」。
- **v1.5.7**：设备信息精简为名称/电量；桥接应用再次上线自动重握手。
- **v1.5.6**：设备信息卡片 + 关于大页面（含开源代码跳转）。
- **v1.5.5**：使用提示卡 + 最近应用 + 关于按钮。
- **v1.5.4**：断开连接按钮 + 通知栏状态统计。
- **v1.5.3**：只显示已配对设备（移除扫描）。
- **v1.5.2**：MIUIX（HyperOS）设计语言 + 跟随系统深浅。
- **v1.5.1**：自适应图标（蓝渐变 + 白链环）。
- **v1.5.0**：后台保活（前台服务 + 唤醒锁）。
- **v1.4.0**：修复明文 HTTP 失败；MD3 风格 UI；记住上次设备。
